...
\mc{The purpose of testing is to exercise a program over its input domain to
validate its functionality on that input or fail. Ideally, tests are
deterministic: they either always pass or always fail.

* Why this is good
* Localisation
* Reproduction

But, some are not deterministic.

Som fail often enough, and are cheap enough that iteration effectively makes
them deterministic.

Some do not -> some are rare.

We call these \emph{\flaky} tests.}
