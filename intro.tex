...
Simply surpressing \flaky tests is not a long-term solution. Informally, let
...
For personal experience, \flaky tests cause confusion, obscure real failures and
...
which we may intervene to aid developers dealing with them:
\begin{enumerate}
...
Existing tools provide crude \flaky test identification. Information from these
tools can be used to inform prioritisation decisions. As of yet, no tool exists
to tackle resolution itself. Ultimately, it is the resolution of \flaky tests
that we are interesed in. In order to tackle the problem practically though,
knowledge gathered at each stage must be combined and considered.
