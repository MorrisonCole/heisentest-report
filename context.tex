...
\begin{defn}[\Flaky{} Test]\label{def:flaky_test}
	Let $\vec{I}$ be the lifting of all inputs, including coin flips and environmental interactions, into a single input vector. Given a \textit{subject under test} $f: \vec{I} \rightarrow \vec{O}$, a \textit{test case} is $(\vec{i},\vec{o})$. A \textit{\flaky{} test case} is a \textit{test case} where, for fixed $f$
	$$p(f(\vec{i}) = \vec{o}) = 1 - \epsilon$$
	for non-negligible $\epsilon$.
\end{defn}

The key is the true $\vec{I}$ is only partially known; we capture flakiness as unknown components of $\vec{I}$. Combined with our budget $B$, we can determine what values of $\epsilon$ we can afford to detect.
