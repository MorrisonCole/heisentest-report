...
Each event is created as a bean in memory and held in a blocking queue until it
...
tests, we should run as part of a modular continuous integration system. The
Jenkins plugin is a wrapper that manages the storage of the test results and
...
Or, we could group the tests by common failure type, \eg:
