...
Projects with a huge number of tests face lengthy build times. It may be in a teams' interest to remove trusted tests from the per-commit build and instead run them on a seperate cycle to tighten the immediate feedback loop. We could provide a listing of 'most trusted' tests (\ie, tests that have exhibited no flakiness over a significant period).
