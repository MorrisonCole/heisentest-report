...
an some test suite $T$ with $\sum\limits_{t \in T} averageTimeToCompletion(t)$.
Given some maximum time $B^{max}$, we have a test suite budget $B$, where
\begin{align*}
B = B^{max} - \sum\limits_{t \in T} averageTimeToCompletion(t)
\end{align*}

Note that a sensible $B^{max}$ will vary with user needs, so it will be exposed
for modification.

It is important that we are able to estimate the effects of individual probes in
terms of timing in order to meet our budget. If our instrumentation causes a
individual test to run over budget, we risk running over budget at the suite
level. We adapt probe costs by comparing expected overhead to measured overhead.

For a total exploratory budget $B_{e}$ and total flaky test budget $B_{f}$, we
have total test suite budget $B$, where
\begin{align*}
B = B_{e} + B_{f}
\end{align*}
...
Test suite budget is distributed over all tests. We partition our budget into
...
\begin{algorithmic}
	...
	\Require{$B_{t}^{max}$ is the maximum targeted budget for a test $t$}
	\Function{calculateTestTargetedBudget}{$test$, $targetedBudget$}

	\State $maximumTargetedBudget \gets t_{b_max}$
