...
\begin{description}
	\item[Total time available] \hfill \\
 		Our upper bound for run time. This could be specified by the developers, or left as a sensible default.
...
\begin{algorithmic}
	...
	\STATE{$targetedBudget = suiteBudget * proportionTargeted$}
	\STATE{$exploratoryBudget = suiteBudget * (1 - proportionTargeted)$}

	\STATE{$totalFlakiness = \text{calculateFlakiness(}tests$)}
	...
	\STATE{shuffle($tests$)}
	\STATE{$iterator = tests.getIterator()$}
