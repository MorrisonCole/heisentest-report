...
\begin{algorithmic}
	...
	\State{$flakyBudget \gets suiteBudget * proportionTargeted$}
	\State{$exploratoryBudget \gets suiteBudget * (1 - proportionTargeted)$}
	\Statex

	\State{allocateTargetedBudget($tests$, $flakyBudget$)}
...
\begin{center}
  \begin{tabular}{| l | p{2.4cm} | l | l |}
	  \hline
	    \textbf{Probe Type} & \textbf{Number of Arguments} & \textbf{Duration
	    (ms)} & \textbf{Increase Scale Factor} \\
	  \hline
			{\tt NONE} & 1 & 0.2626 & 1 (Baseline) \\
		\hline
	    {\tt SIMPLE} & 1 & 24.17 & 92 \\
	  \hline
	    {\tt COMPLEX} & 1 & 67.27 & 256 \\
	  \hline
  \end{tabular}
\end{center}
