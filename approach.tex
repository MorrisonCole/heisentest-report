...
Given a test $t$ with probability of failure $P_{t}$, test \flakiness is
calculated by
\begin{align*}
  \mathit{flakiness}(P_{t})=
	\begin{cases}
	    1 - \frac{P_{t}}{\alpha},& \text{if } P_{t}< \alpha\\
	    0,              & \text{otherwise}
	\end{cases}
\end{align*}
...
budget, but not guaranteed. The pseudocode algorithms below define our exact
methods for allocating and instrumenting a test suite.

\begin{algorithm}[H]
\caption{Instrumenting the test suite with respect to a budget}
\label{alg:InstrumentTestSuite}
...
\begin{algorithmic}
	...
	\Function{InstrumentTestSuite}{$T$, $proportionFlaky$}
...
\algref{alg:InstrumentTestSuite} defines the two main steps: allocation and
instrumentation. First, $B_t$ is calculated for each $t$. Then, the
instrumentation is performed. It is during the instrumentation step that the
application is actually modified.
...
and predicates used in past bug isolation work and instead include
instrumentation designed to gather much more contextual information.
...
the uninstrumented test with 1 argument --- the others are similar. Timings were
...
If we have no historical data, we simply take the next set of code units in the
control flow graph following the entry point --- in our case, a test set up or
test method.

The main strategy is to place probes further and further down the control
flow graph until we are able to detect a failure-predicting predicate.

Once we have detected a failure-predicting predicate, we can assign the majority
of our budget to drill down in related areas. Still, we reserve a portion of our
individual test budget to explore in case we get lucky and find another failure
predicting predicate elsewhere. This is similar to our approach on the test
suite level, where we split our main budget $B$ into \flaky budget $B_f$ and
exploratory budget $B_e$.
...
$B_f$ allocated to the test until $\epsilon$ returns to its previous value. If
$\epsilon$ never returns and the test is stable (\ie, $\epsilon$ is $0$), we
assume it was fixed, provided there was a associated source code change when the
behaviour originally changed.
