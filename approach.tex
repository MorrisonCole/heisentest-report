...
Each test can be assigned a flakiness value in the range $[0-1]$, representing its chance of failure. This is used to determine test budget allocation.
...
\begin{center}
	...
	\begin{tikzpicture}[thick, framed, x=6.5cm, y=1.6cm]
	    ...
	    % Flakiness label
	    \node [above] at (0.2, 0.5) {$Flakiness$};
...
Flakiness is calculated with respect to the threshold $\alpha$.
...
\begin{algorithmic}
	...
	\IF{$chanceOfFailure \geq threshold$}
		\RETURN{$0$}
	\ELSE
