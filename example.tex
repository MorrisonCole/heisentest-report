...
The next step would be to look at the test result page for that device, and that
test. Thankfully, the Flaky Test monitor is integrated with the test runner
results, and links us straight there.
...
\begin{itemize}
	...
	\item Presuming the test is named correctly (\quote{testThatShareIsAbsent}),
	the screenshot seems to be in order --- we can see no {\lq}Share{\rq} icon on
	the page.
\end{itemize}
